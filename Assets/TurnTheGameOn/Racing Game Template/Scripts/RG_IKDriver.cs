...
//	public RG_SyncData syncVars;
	...
	void Update(){
		if (netID == null || gameMode == "SINGLE PLAYER") {
			if (mobile) {
			...
			if (!netID.isLocalPlayer) {
				//horizontalInput = syncVars.horizontalInput;
				//verticalInput = syncVars.verticalInput;
			} else {
				if (mobile) {
					horizontalInput = UnityStandardAssets.CrossPlatformInput.CrossPlatformInputManager.GetAxis ("Horizontal");
					verticalInput = UnityStandardAssets.CrossPlatformInput.CrossPlatformInputManager.GetAxis ("Vertical");
			...
			/*if (//gearString != syncVars.gearString) {
				gearString = syncVars.gearString;*/
				TargetShifter ();
		...
		if (steeringWheel != null) {
			...
			steeringWheel.localEulerAngles = new Vector3 (defaultSteering.x, defaultSteering.y, zAngle);//temp2;//new Vector3 (30.8f, 0, -(Input.GetAxis ("Horizontal") * steeringWheelRotation));

		}
	}

	public void GetGearText(){
		if (gearText != null) {
		...
		if (animator) {			
			if (ikActive) {
				...
				if (rightHandObj != null) {
					...
					if (shifting == false) {
						targetRightHandIK.localPosition = Vector3.Lerp (targetRightHandIK.localPosition, rightHandObj.localPosition, 8 * Time.deltaTime);
